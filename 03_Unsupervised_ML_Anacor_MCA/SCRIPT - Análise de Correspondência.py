...
result = pyreadr.read_r('datasets/perfil_investidor_aplicacao.RData') 
print(result.keys()) # Checando o nome do DataFrame

#%%
# Carregando o DataFrame
df = result["perfil_investidor_aplicacao"]
...
print('####### FREQUENCY OF CATEGORY AT THE VARIABLES #########')
print(df[['perfil']].value_counts())
print('########################################################')
print(df[['aplicacao']].value_counts())
...
tabela_contingencia = pd.crosstab(index=df['perfil'], columns=df['aplicacao'])
...
expected = expected.rename_axis('perfil').rename_axis('aplicacao', axis='columns')
...
result = pyreadr.read_r('perfil_investidor_aplicacao_estadocivil.RData') 
print(result.keys()) # Checando o nome do DataFrame

#%%
# Carregando o DataFrame
df = result["perfil_investidor_aplicacao_estadocivil"]
...
tabela_contingencia = pd.crosstab(index=df['perfil'], columns=df['estado_civil'])
print('####### CONTINGENCY TABLE OBSERVED ##########')
print(tabela_contingencia)
...
#%%
# Tabelas de contingência
# Aplicacao x Estado Civil

tabela_contingencia = pd.crosstab(index=df['aplicacao'], columns=df['estado_civil'])
print('####### CONTINGENCY TABLE OBSERVED ##########')
print(tabela_contingencia)
...
# Possivel de serem feitas mas não existem rotinas já implementadas no Python
...
ax = mca.plot_coordinates(df.iloc[:, 1:],  ax=None, figsize=(6, 6),show_row_points=False,
                             show_column_points=True, column_points_size=30, show_column_labels=True, legend_n_cols=1)
ax.get_figure()

print(mca.row_coordinates(df.iloc[:, 1:]))
print(mca.column_coordinates(df.iloc[:, 1:]))
