...
df = pd.read_excel('notas_fatorial.xlsx') 
...
# Aqui está diferente do que no R, investigar depois o que realmente o geom_smooth faz
model = LinearRegression()
x = df['custos'].values.reshape(-1, 1)
y = df['finanças'].values.reshape(-1, 1)
model.fit(x, y)
...
model = LinearRegression()
x = df['custos'].values.reshape(-1, 1)
y = df['marketing'].values.reshape(-1, 1)
model.fit(x, y)
...
model = LinearRegression()
x = df['custos'].values.reshape(-1, 1)
y = df['atuária'].values.reshape(-1, 1)
model.fit(x, y)
