...
#%%
# Carregamento da base de dados (indicador_país)
df = pd.read_excel('indicador_país.xlsx') 
...
column_x = 'escol1'
column_y = 'pib_capita1'

x = df[column_x].values.reshape(-1, 1)
y = df[column_y].values.reshape(-1, 1)
model.fit(x, y)
...
#%%
# Scatter e ajuste linear entre as variáveis 'escolaridade' e 'violencia'
...
corr_coef, corr_sig = pearson(df.iloc[:,1:5], df.iloc[:,1:5])
...
bartlett_chisq, bartlett_degrees, bartlett_pvalue = bartlett_sphericity(df.iloc[:, 1:5], 'pearson')
...
#%%
# Z-scores
df_zscore = zscore(df.iloc[:,1:5], ddof = 1)
print('##### Z-SCORES ######') #OK
print(df_zscore)
...
#%%
cargas_fatoriais, _ = pearson( df.iloc[:,1:5], fatores)
